...
module PVOutput
  ...
  class Statistic
    ...
    def self.fetch (date_from, date_to, auth_details)
      ...
      raw_data = RestClient.get(API_URL, {:params => {:sid => auth_details.system_id, :key => auth_details.api_key, :df => date_from, :dt => date_to, :c => '1', :crdr => '1'}})
      results = CSV.new(raw_data).to_a
